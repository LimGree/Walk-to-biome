...
public class Constructor : BuildingBase, IInteractable
{
    [Header("Constructor Settings")]
    ...
    public override bool TryReceiveItem(ItemData item, BuildingSocket fromSocket)
    {
        ...
        if (showDebug)
            Debug.Log($"Constructor получил: {item.displayName}. Теперь: {inputBuffer[item]}");
        ...
        for (int i = 0; i < currentRecipe.outputs.Count; i++)
        {
            ...
            for (int n = 0; n < output.amount; n++)
            {
                if (!TryOutputToAny(output.item) && showDebug)
                    Debug.LogWarning($"Constructor: буфер переполнен при выдаче {output.item?.displayName}");
                else if (showDebug)
                    Debug.Log($"Constructor выдал: {output.item.displayName}");
            }
        }
